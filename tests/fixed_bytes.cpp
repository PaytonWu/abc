...
{
    fixed_bytes<2, ByteNumbering::Lsb0> bytes;
    for (uint16_t i : std::views::iota(static_cast<uint16_t>(0u), std::numeric_limits<uint16_t>::max()))
    {
...
{
    fixed_bytes<2, ByteNumbering::Msb0> bytes;
    for (uint16_t i : std::views::iota(static_cast<uint16_t>(0u), std::numeric_limits<uint16_t>::max()))
    {
        ASSERT_EQ(i, bytes.to<uint16_t>());
        ++bytes;
...
//{
//     auto bytes = hex_string::from("0x1234567890").transform([](auto && hexstr) { return hexstr.template bytes<ByteNumbering::msb0>(); })
//                                                  .value();
...
{
    auto const & result = hex_string::from("0x123456789012345678901234567890")
                              .transform([](auto && hex) { return hex.template bytes<ByteNumbering::Msb0>(); })
                              .and_then([](auto && bytes) { return fixed_bytes<15, ByteNumbering::Msb0>::from<ByteNumbering::Msb0>(bytes); })
...
{
    auto result = hex_string::from("0x123456789012345678901234567890")
                      .transform([](auto && hex) { return hex.template bytes<ByteNumbering::Msb0>(); })
                      .and_then([](auto && bytes) { return fixed_bytes<15, ByteNumbering::Msb0>::from<ByteNumbering::Msb0>(bytes); })
...
{
    auto bytes32 = hex_string::from("0x123456789012345678901234567890")
                       .transform([](auto && hex) { return hex.template bytes<ByteNumbering::Msb0>(); })
                       .and_then([](auto && bytes) { return bytes32_be_t::from<ByteNumbering::Msb0>(bytes); })
                       .value();
    // 5678901234567890
...
{
    auto bytes32 = hex_string::from("0x123456789012345678901234567890")
                       .transform([](auto && hex) { return hex.template bytes<ByteNumbering::Lsb0>(); })
                       .and_then([](auto && bytes) { return bytes32_be_t::from<ByteNumbering::Lsb0>(bytes); })
                       .value();
    // 5678901234567890
    EXPECT_EQ(0x56, bytes32[24]);
    EXPECT_EQ(0x78, bytes32[25]);
...
TEST(fixed_bytes, le_from_be)
{
    auto bytes32 = hex_string::from("0x123456789012345678901234567890")
                       .transform([](auto && hex) { return hex.template bytes<ByteNumbering::Msb0>(); })
                       .and_then([](auto && bytes) { return bytes32_le_t::from<ByteNumbering::Msb0>(bytes); })
                       .value();
...
TEST(fixed_bytes, le_from_le)
{
    auto bytes32 = hex_string::from("0x123456789012345678901234567890")
                       .transform([](auto && hex) { return hex.template bytes<ByteNumbering::Lsb0>(); })
                       .and_then([](auto && bytes) { return bytes32_le_t::from<ByteNumbering::Lsb0>(bytes); })
                       .value();
...
{
    auto bytes15 =
        hex_string::from("0x123456789012345678901234567890").transform([](auto && hex) { return hex.template bytes<ByteNumbering::Msb0>(); }).and_then([](auto && bytes) {
            return fixed_bytes<15, ByteNumbering::Msb0>::from<ByteNumbering::Msb0>(bytes);
        });
    ASSERT_TRUE(bytes15.has_value());
    auto const span8 = bytes15.and_then([](auto && bytes) { return bytes.subspan(7); }).value();
    // 5678901234567890
    EXPECT_EQ(0x56, span8[0]);
    EXPECT_EQ(0x78, span8[1]);
    EXPECT_EQ(0x90, span8[2]);
    EXPECT_EQ(0x12, span8[3]);
    EXPECT_EQ(0x34, span8[4]);
    EXPECT_EQ(0x56, span8[5]);
    EXPECT_EQ(0x78, span8[6]);
...
{
    {
        auto result = hex_string::from("0x123456789012345678901234567890")
                          .transform([](auto && hex) { return hex.template bytes<ByteNumbering::Msb0>(); })
                          .and_then([](auto && bytes) { return fixed_bytes<15, ByteNumbering::Msb0>::from<ByteNumbering::Msb0>(bytes); })
                          .and_then([](auto && bytes) { return std::forward<std::remove_reference_t<decltype(bytes)>>(bytes).subspan(7); });
        ASSERT_TRUE(result.is_err());
    }

    {
        auto result = hex_string::from("0x123456789012345678901234567890")
                          .transform([](auto const && hex) { return hex.template bytes<ByteNumbering::Msb0>(); })
                          .and_then([](auto const && bytes) { return fixed_bytes<15, ByteNumbering::Msb0>::from<ByteNumbering::Msb0>(bytes); })
                          .and_then([](auto const && bytes) { return std::forward<std::remove_reference_t<decltype(bytes)> const>(bytes).subspan(7); });
        ASSERT_TRUE(result.is_err());
    }
