...
TEST(fixed_bytes, copy_constructor) {
    ...
    bytes[0] = 1;
    bytes[15] = 2;
    fixed_bytes16_msb0_t bytes2{ bytes };
...
TEST(fixed_bytes, move_constructor) {
    ...
    bytes[0] = 1;
    bytes[15] = 2;
    fixed_bytes16_msb0_t bytes2{ std::move(bytes) };
    EXPECT_EQ(bytes2.size(), 16);
    EXPECT_EQ(bytes2[0], 1);
    EXPECT_EQ(bytes2[15], 2);
}
...
TEST(fixed_bytes, initializer_list_constructor) {
    fixed_bytes16_msb0_t bytes{ std::array<byte, 16>{ 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f } };
    EXPECT_EQ(bytes.size(), 16);
    EXPECT_EQ(bytes[0], 0);
    EXPECT_EQ(bytes[7], 7);
}
...
TEST(fixed_bytes, array_constructor) {
    ...
    fixed_bytes16_msb0_t bytes{ arr };
    EXPECT_EQ(bytes.size(), 16);
...
TEST(fixed_bytes, from) {
    fixed_bytes16_msb0_t bytes{ 0x0102030405060708U };
    EXPECT_EQ(bytes.size(), 16);
    EXPECT_EQ(bytes[0], 0);
    EXPECT_EQ(bytes[7], 0);
    EXPECT_EQ(bytes[8], 1);
...
TEST(fixed_bytes_le, from) {
    fixed_bytes16_lsb0_t bytes{ 0x0102030405060708U };
    EXPECT_EQ(bytes.size(), 16);
    EXPECT_EQ(bytes[0], 8);
    EXPECT_EQ(bytes[7], 1);

    fixed_bytes4_lsb0_t bytes2{ 0x0102030405060708U };
...
TEST(fixed_bytes_be, from) {
    ...
    fixed_bytes4_msb0_t bytes2{ 0x0102030405060708U };
    EXPECT_EQ(bytes2.size(), 4);
...
TEST(fixed_bytes_le, to) {
    fixed_bytes16_lsb0_t bytes{ std::array<byte, 16>{ 8, 7, 6, 5, 4, 3, 2, 1 } };
...
TEST(fixed_bytes_be, to) {
    fixed_bytes16_msb0_t bytes{ std::array<byte, 16>{ 1, 2, 3, 4, 5, 6, 7, 8 } };  // 0x01020304050607080000000000000000
...
TEST(fixed_bytes_be, hex_string) {
    auto bytes = fixed_bytes16_msb0_t{0x0123456789abcdefu};
...
TEST(fixed_bytes_le, hex_string) {
    auto bytes = fixed_bytes16_lsb0_t{0x0123456789abcdefu};
    auto const & hex_str = to_hex_string(bytes);
    EXPECT_EQ("0x00000000000000000123456789abcdef", hex_str.to_string());
}