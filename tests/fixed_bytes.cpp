...
TEST(fixed_bytes, initializer_list_constructor) {
    bytes16_msb0_t bytes{ std::array<byte, 16>{ 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f } };
    EXPECT_EQ(bytes.size(), 16);
    EXPECT_EQ(bytes[0], 0);
    EXPECT_EQ(bytes[7], 7);
}
...
TEST(fixed_bytes_le, to) {
    bytes16_lsb0_t bytes{ std::array<byte, 16>{ 8, 7, 6, 5, 4, 3, 2, 1 } };
...
TEST(fixed_bytes_be, to) {
    bytes16_msb0_t bytes{ std::array<byte, 16>{ 1, 2, 3, 4, 5, 6, 7, 8 } };  // 0x01020304050607080000000000000000
