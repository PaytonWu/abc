...
TEST(expected, copy_constructor)
{
    struct non_trivally_copyable
    {
        ...
        non_trivally_copyable & operator=(non_trivally_copyable const &) = default;
...
TEST(expected, move_constructor)
{
    struct non_trivally_moveable
    {
        ...
        non_trivally_moveable & operator=(non_trivally_moveable const &) = default;
...
TEST(expected, map_non_copyable_value)
{
    ...
    {
        auto r2 = static_cast<abc::expected<noncopyable, int> const &&>(abc::expected<noncopyable, int>{ abc::unexpected<int>{ 1 } }).map([](auto const && v) {
            return std::move(v).value().length() + 1;
        });
        ASSERT_FALSE(r2.has_value());
        ASSERT_EQ(1, r2.error());
    }
...
constexpr auto
hex_string_to_binary(std::string_view string_slice) -> abc::expected<abc::Bytes<abc::EndianType<Endian>::byte_numbering_value>, abc::errc>
{
    ...
    abc::Bytes<abc::EndianType<Endian>::byte_numbering_value> binary_data;
...
from_hex_prefixed(std::string_view input) -> abc::expected<abc::hex_string, abc::errc>
{
    ...
    return hex_string_to_binary<std::endian::big>(input).transform([](auto && bytes) {
        ...
        return abc::hex_string::from<abc::ByteNumbering::Msb0>(std::move(bytes));
    });
}
...
{
    {
        auto r1 = wrapper::hex_string_to_binary<std::endian::big>("0x1234").transform([](auto && v) {
            ranges::reverse(v);
            return v;
        });
        ASSERT_TRUE(r1.has_value());
